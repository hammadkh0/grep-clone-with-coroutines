...
func discoverDirs(wl *worklist.Worklist, path string) {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			discoverDirs(wl, nextPath) // recurse into folders untill no more folders or files left to search
		} else {
...
func main() {
	...
	go func() {
		workersWg.Wait()
		close(blockWorkersWg) // close the channel
	}()
	...
	go func() {
		for {
			...
			case <-blockWorkersWg: // when channel is closed this case will always succeed
				if len(results) == 0 {
					displayWg.Done()
					return
				}
